...
import { loadMap, setCurrentMap, getCurrentMap, MapLoader } from './maploader.js';

if (!window.factory) {
  try {
    registerDefaultPrefabs();
  } catch (e) {
    console.warn('registerDefaultPrefabs()', e);
  }
...
function makeFallbackTiles() {
  ...
  for (let yy = 1; yy <= 10; yy++) {
    for (let xx = 1; xx <= 12; xx++) t[yy][xx] = 0;
  }
  for (let yy = 8; yy <= 10; yy++) {
    for (let xx = 12; xx <= 24; xx++) t[yy][xx] = 0;
  }
  for (let yy = 8; yy <= 9; yy++) {
    for (let xx = 22; xx <= 24; xx++) t[yy][xx] = 2;
  }
  t[9][15] = 0;
  return t;
}
...
renderer.init().then(async () => {
    ...
    try {
      ...
      for (let y = 0; y < tiles.length; y++) {
        for (let x = 0; x < tiles[y].length; x++) {
          ...
          g.x = x * TS;
          g.y = y * TS;
          cont && cont.addChild(g);
        }
      }
    } catch (_) {
      /* fallback draw best effort */
    }
  }
  ...
  const mover = playerNode.addComponent(
    new MoveScript(input, () => {
    ...
    for (const c of getColliders()) {
      ...
      if (overlapX > 0 && overlapY > 0) {
        if (overlapX < overlapY) {
          if (rx < b.x) rx -= overlapX;
          else rx += overlapX;
        } else {
          if (ry < b.y) ry -= overlapY;
          else ry += overlapY;
        }
      }
    }
  ...
  class TileAndEntityCollisionResolver extends Component {
    onUpdate() {
      ...
      let res = moveWithTileCollisions(
        playerNode,
        mover.dx || 0,
        mover.dy || 0,
        tiles,
  ...
  factory.register('crate', {
    ...
    layer: 'default',
    props: { loot: 1 },
    collider: { size: 18, solid: true }
  });
  factory.register('gem', {
    sprite: 'rect:24',
    layer: 'default',
    props: { value: 10 },
    collider: null
  });
  factory.register('bot', {
    sprite: 'rect:16',
    layer: 'default',
    props: { hp: 5 },
    collider: { size: 16, solid: true }
  });
  ...
  for (const n of scene.children) {
    ...
    if (nm.includes('bot')) {
      n.addComponent(
        new PatrolBehavior({ axis: 'x', from: 18 * TS, to: 24 * TS, speed: 0.1, pauseMs: 300 })
      );
    }
  }
  ...
  triggers.add(
    zone(3, 2, 3, 2, {
      ...
      once: true,
      onEnter: () => {
        lastEvent = 'Map active • Use WASD/Arrows';
      }
    })
  );
  class TriggerDriver extends Component {
    onUpdate() {
      ...
      triggers.check(rect, trig => {
        if (trig.onEnter) trig.onEnter(trig);
      });
    }
  }
  ...
  if (_data) {
    ...
    if (Array.isArray(_data.entities)) {
      for (const e of _data.entities) {
        try {
          const node = window.factory.spawn(e.type, {
            x: (e.x || 0) * TS,
            y: (e.y || 0) * TS,
            layer: 'actors'
          });
          ...
          console.log(
            ...
            e.type,
            'at',
            node && node.transform && node.transform.position,
            'layer=actors'
          );
        } catch (err) {
...
try {
  for (let i = 0; i < 3; i++) {
    ...
    if (n && n.get) {
      try {
        ...
        if (s && 'tint' in s) s.tint = 0x5edfff;
      } catch {}
    }
    scene.add(n);
  }
  ...
  console.warn('[debug] marker spawn failed', e);
}
