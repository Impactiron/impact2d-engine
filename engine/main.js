// !mpact2d — v0.8 (Render tiles after load if layer empty) • 2025-11-01
...
const BUILD = "V0.8-RENDER-TILES-AFTER-LOAD-2025-11-01";
...
renderer.defineLayer('default', 1.0);
  ...
  class TileAndEntityCollisionResolver extends Component {
    onUpdate(){
      const tr = playerNode.getComponent(Transform);
      if(!tiles){ tr.position.x += mover.dx||0; tr.position.y += mover.dy||0; const spr = playerNode.getComponent(Sprite); if(spr&&spr._pixi){ spr._pixi.x=tr.position.x; spr._pixi.y=tr.position.y; } return; }
  ...
  function drawTilesGrid(data) {
    const pal = data?.palette || { 0:0x202733, 1:0x586174, 2:0xb24b36, 3:0x2f6aa5, 4:0xa68a5b };
    const cont = renderer.getLayerContainer('world');
    cont.removeChildren(); // ensure clean
    for (let y=0; y<tiles.length; y++) {
      for (let x=0; x<tiles[y].length; x++) {
        const v = tiles[y][x]; const col = pal[v] ?? 0x333333;
  ...
  try {
    ...
    // If MapLoader didn't render tiles, draw them here
    const cont = renderer.getLayerContainer('world');
    if (!tiles || !tiles.length) { throw new Error('No tiles in JSON'); }
    if (cont.children.length === 0) { drawTilesGrid(data); }
    // Entities from JSON
    ...
    TS = 32;
    const W=40,H=30; tiles=[]; const pal={0:0x202733,1:0x586174,2:0xb24b36,3:0x2f6aa5,4:0xa68a5b};
    for(let y=0;y<H;y++){ const row=[]; for(let x=0;x<W;x++){ let v=0; if(x===0||y===0||x===W-1||y===H-1) v=1; else if((x+y)%19===0) v=1; else if((x*y)%113===0) v=2; else if((x+2*y)%37===0) v=3; else if((2*x+y)%41===0) v=4; row.push(v); } tiles.push(row);}
    drawTilesGrid({palette:pal});
