...
renderer.init().then(async ()=>{
  ...
  class TriggerDriver extends Component { onUpdate(){ const tr = playerNode.getComponent(Transform); const rect = { x: tr.position.x, y: tr.position.y, w: PLAYER_SIZE, h: PLAYER_SIZE }; triggers.check(rect, (trig) => { if (trig.onEnter) trig.onEnter(trig); }); } }
